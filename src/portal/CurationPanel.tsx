...
export default function CurationPanel({detail,user,onSaved}:{detail:CaseDetails;user:TeamUser;onSaved:(data:CaseDetails)=>void}) {
  ...
  return <div className="pc-panel">
    <div className="pc-summary"><div><span className="ps-overline">CURADORIA · {c.legacy?'RÉGUA ANTERIOR':'SELECT V2'}</span><h3>Evidências antes da decisão.</h3><p>{c.legacy?'Qualidade e mercado, conforme a política preservada deste dossiê.':'Qualidade do imóvel, sem faixa mínima de preço. Mercado e documentação são conferidos separadamente.'} Avaliação preenchida pela equipe; IA ainda não conectada.</p></div><div className="pc-score"><strong>{c.score??'—'}<small>/100</small></strong><span>{c.coverage}/{c.criteria.length} dimensões preenchidas</span></div></div>
