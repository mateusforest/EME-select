...
export default function CurationPanel({detail,user,onSaved}:{detail:CaseDetails;user:TeamUser;onSaved:(data:CaseDetails)=>void}) {
  ...
  return <div className="pc-panel">
    <div className="pc-summary"><div><span className="ps-overline">CURADORIA · RÉGUA PILOTO</span><h3>Evidências antes da decisão.</h3><p>Qualidade e mercado, sem faixa mínima de preço. Avaliação preenchida pela equipe; IA ainda não conectada.</p></div><div className="pc-score"><strong>{c.score??'—'}<small>/100</small></strong><span>{c.coverage}/4 dimensões documentadas</span></div></div>
    <p className="pc-policy">{c.policy} · hipótese de corte em 80/100, mínimo 3/5 por dimensão. Proposta para calibrar com a equipe; não é um padrão certificado do mercado. A nota exibida considera a última versão salva.</p>
    ...
    <form className="ps-form" onSubmit={save}>
      ...
      <section><div className="pc-section-title"><ClipboardCheck size={18}/><h3>Qualidade e mercado</h3></div><p className="pc-help">0 crítica · 1 inadequada · 2 abaixo do padrão · 3 atende · 4 boa · 5 superior. Use “Não verificado” quando faltar evidência.</p>
      <div className="pc-criteria">{criteria.map((item,index)=><div className="pc-criterion" key={item.key}><div className="pc-criterion-head"><strong>{item.label}</strong><span>Peso {item.weight}%</span></div><p>{item.help}</p><label>Nota · {item.label}<select value={item.score??''} onChange={e=>{setCriteria(criteria.map((v,i)=>i===index?{...v,score:e.target.value===''?null:Number(e.target.value)}:v));setDirty(true);}}><option value="">Não verificado</option>{[0,1,2,3,4,5].map(n=><option key={n} value={n}>{n}/5</option>)}</select></label><label>Evidência · {item.label}<textarea rows={3} maxLength={1800} minLength={item.score===null?0:10} required={item.score!==null} placeholder="Descreva o observado, a fonte e a data. Diferencie declaração de verificação." value={item.note} onChange={e=>{setCriteria(criteria.map((v,i)=>i===index?{...v,note:e.target.value}:v));setDirty(true);}}/></label></div>)}</div></section>
      <section className="pc-checks"><div className="pc-section-title"><ShieldCheck size={18}/><h3>Verificações para a entrada</h3></div><p className="pc-help">Registre o escopo, a fonte, a data e quem realizou a conferência. Os arquivos ainda devem permanecer no repositório privado da equipe; esta etapa guarda referências textuais, sem upload ou consulta automática. Somente o administrador conclui uma conferência.</p>
      {checks.map((item,index)=><div key={item.key} className="pc-check"><label>{item.label}<select disabled={!admin&&item.state==='Conferido'} value={item.state} onChange={e=>{setChecks(checks.map((v,i)=>i===index?{...v,state:e.target.value}:v));setDirty(true);}}><option>Pendente</option><option>Em revisão</option><option disabled={!admin}>Conferido</option></select></label><label>Referência · {item.label}<textarea rows={2} disabled={!admin&&item.state==='Conferido'} maxLength={1800} required={item.state==='Conferido'} minLength={item.state==='Conferido'?10:0} value={item.note} onChange={e=>{setChecks(checks.map((v,i)=>i===index?{...v,note:e.target.value}:v));setDirty(true);}}/></label>{item.author&&<small>Último registro: {item.author} · {item.date&&new Date(item.date).toLocaleString('pt-BR')}</small>}</div>)}
