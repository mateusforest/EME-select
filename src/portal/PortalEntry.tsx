...
export default function PortalEntry() {
  const [session,setSession]=useState<{user:TeamUser|null;needsSetup:boolean}|null>(null),[connectionError,setConnectionError]=useState(''),[serviceMissing,setServiceMissing]=useState(false),[reload,setReload]=useState(0);
  ...
  useEffect(()=>{if(isDemo)return;let active=true;setConnectionError('');setServiceMissing(false);api<{user:TeamUser|null;needsSetup:boolean}>('/auth/session').then(value=>{if(active)setSession(value);}).catch(error=>{if(active){setServiceMissing(!isLocal&&error instanceof ApiError&&error.status===404);setConnectionError(error.message);}});return()=>{active=false;};},[reload]);
