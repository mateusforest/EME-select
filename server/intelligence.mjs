...
export async function requestAnalysis({ apiKey, model, task, context, instruction, editorial=false, fetcher = fetch }) {
  ...
  if(editorial&&(result.draftReply.length<80||result.draftReply.length>5000||result.strengths.join('\n').length>1200||result.nextActions.join('\n').length>1200))aiFail(502,'A sugestão ultrapassou o formato do anúncio. Tente novamente.');
