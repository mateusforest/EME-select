 ...
 async function handle(req,res){
  ...
  try{
   ...
   if(match[1]==='evaluations'){
    ...
    else if(action==='decision'&&req.method==='POST'){data=decision(data,body,user);label=data.stage;note=body.reason;}
    ...
    if(!action&&req.method==='PATCH'){fields(body,['version','draft','photos']);const d=draft(body.draft);if(!Array.isArray(body.photos)||body.photos.length>20||new Set(body.photos.map(p=>p?.id)).size!==body.photos.length)fail(400,'Galeria inválida.');const photos=body.photos.map(p=>{fields(p,['id','caption']);const original=data.photos.find(x=>x.id===p.id);if(!original)fail(400,'Foto não pertence ao cadastro.');return {...original,caption:text(p.caption,0,180)};});data=changed({...syncDraft(data,d),photos});}
     ...
     fields(body,['version','content','caption']);const caption=text(body.caption||'',0,180);await rate('photo:'+user.id,100);if(data.photos.length>=20)fail(400,'Limite de 20 fotos.');if(typeof body.content!=='string'||body.content.length>2800000||!/^data:image\/(webp|jpeg|png);base64,[A-Za-z0-9+/]+=*$/.test(body.content))fail(400,'Prepare a imagem antes do envio.');const bytes=Buffer.from(body.content.split(',')[1],'base64');let output;try{const image=sharp(bytes,{limitInputPixels:20000000,failOn:'warning'});const m=await image.metadata();if(m.pages>1||!['webp','jpeg','png'].includes(m.format)||Math.max(m.width,m.height)<600||Math.min(m.width,m.height)<200)fail(400,'Fotografia inválida ou pequena demais.');output=await image.rotate().resize({width:2400,height:2400,fit:'inside',withoutEnlargement:true}).webp({quality:88}).toBuffer({resolveWithObject:true});}catch(e){if(e.status)throw e;fail(400,'Não foi possível preparar a foto.');}if(output.data.length>3000000)fail(400,'Reduza o tamanho da foto.');const id=randomUUID();await request('/storage/v1/object/eme-property-photos/'+id+'.webp',{method:'POST',raw:true,body:output.data,headers:{'Content-Type':'image/webp','x-upsert':'false'}});data=changed({...data,photos:[...data.photos,{id,width:output.info.width,height:output.info.height,caption}]});try{row=await save(req,row,data,'Fotografia adicionada','Nova fotografia recebida. Curadoria requer nova conferência.');}catch(e){try{await request('/storage/v1/object/eme-property-photos',{method:'DELETE',body:{prefixes:[id+'.webp']}});}catch{console.error('Private orphan photo requires cleanup');}throw e;}return send(201,listing(row));
    }else fail(405,'Operação indisponível.');
   }
