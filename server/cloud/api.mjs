import {createPeopleApi,peopleRecordId} from '../people.mjs';
...
export function createCloudApi({env=process.env,client=createClient(env)}={}){
 ...
 const people=createPeopleApi({
  read:async()=>(await rest('eme_cases?id=eq.'+peopleRecordId))[0]||null,
  write:async(req,row,data,user,action)=>save(req,row||{id:peopleRecordId,version:0,assignee_id:user.id},data,action,'Apresentação pública das pessoas da EME.',user.id),
  putImage:(id,bytes)=>request('/storage/v1/object/eme-property-photos/person-'+id+'.webp',{method:'POST',raw:true,body:bytes,headers:{'Content-Type':'image/webp','x-upsert':'false'}}),
  getImage:async id=>Buffer.from(await (await request('/storage/v1/object/authenticated/eme-property-photos/person-'+id+'.webp',{raw:true})).arrayBuffer()),
  deleteImage:id=>request('/storage/v1/object/eme-property-photos',{method:'DELETE',body:{prefixes:['person-'+id+'.webp']}}),
 });
 ...
 async function caseFor(id,u){const row=(await rest('eme_cases?id=eq.'+id))[0];if(!row||['development','directory'].includes(row.data.kind)||u.role!=='admin'&&row.assignee_id!==u.id)fail(404,'Imóvel não encontrado.');return row;}
 ...
 async function handle(req,res){
  ...
  try{
   ...
   if(await people.publicHandle(path,req,res,send))return;
   ...
   if(photoMatch&&req.method==='GET'){
    const rows=await rest('eme_cases?data->photos=cs.'+encodeURIComponent(JSON.stringify([{id:photoMatch[1]}])));const row=rows[0];if(!row)fail(404,'Foto não encontrada.');const originalRequested=new URL(req.url,'https://eme.local').searchParams.get('original')==='1';const isPublic=!originalRequested&&row.data.stage==='Entrada aprovada'&&row.data.published?.images.some(p=>p.url==='/api/photos/'+photoMatch[1]);if(!isPublic&&(!user||user.must_change||user.role!=='admin'&&user.id!==row.assignee_id))fail(404,'Foto não encontrada.');
    const response=await request('/storage/v1/object/authenticated/eme-property-photos/'+(originalRequested?(row.data.photos.find(p=>p.id===photoMatch[1])?.originalId||photoMatch[1]):photoMatch[1])+'.webp',{raw:true});const bytes=Buffer.from(await response.arrayBuffer());res.setHeader('Content-Type','image/webp');res.end(bytes);return;
   }
   ...
   if(await people.handle(path,req,res,user,body,send))return;
   ...
   if(['/api/evaluations','/api/listings'].includes(path)){
    if(req.method==='GET'){const rows=(await rest('eme_cases?order=updated_at.desc'+(user.role==='admin'?'':'&assignee_id=eq.'+user.id))).filter(r=>!['development','directory'].includes(r.data.kind));if(path.endsWith('listings'))return send(200,{listings:rows.filter(r=>r.data.draft).map(listing)});const people=await members();return send(200,{evaluations:rows.map(r=>evaluation(r,people))});}
   ...
   if(prepare&&req.method==='POST'){
    fields(body,['version','sourceId']);const row=await caseFor(prepare[1],user);checkVersion(row,body);
    const source=row.data.photos?.find(p=>p.id===body.sourceId);if(!source)fail(404,'Fotografia não encontrada.');await rate('prepare-photo:'+user.id,40);
    const response=await request('/storage/v1/object/authenticated/eme-property-photos/'+(source.originalId||source.id)+'.webp',{raw:true});
    return send(200,await prepareEnlargement(Buffer.from(await response.arrayBuffer())));
   }
   ...
   if(match[1]==='evaluations'){
    ...
    if(!action&&req.method==='PATCH'){fields(body,['version','draft','photos']);const d=draft(body.draft);if(!Array.isArray(body.photos)||body.photos.length>20||new Set(body.photos.map(p=>p?.id)).size!==body.photos.length)fail(400,'Galeria inválida.');const photos=body.photos.map(p=>{fields(p,['id','caption','room']);const original=data.photos.find(x=>x.id===p.id);if(!original)fail(400,'Foto não pertence ao cadastro.');return {...original,caption:text(p.caption,0,180),room:text(p.room??'',0,60)};});data=changed({...syncDraft(data,d),photos});}
     ...
     fields(body,['version','content','caption','room','sourceId','method','reviewed']);const source=body.sourceId?data.photos.find(p=>p.id===body.sourceId):null;if(body.sourceId&&(!source||!['esrgan-slim-2x','lanczos3-2x'].includes(body.method)||body.reviewed!==true))fail(400,'Confira a foto de origem e confirme a fidelidade da ampliação.');const caption=text(body.caption||'',0,180),room=text(body.room??'',0,60);await rate('photo:'+user.id,100);if(!source&&data.photos.length>=20)fail(400,'Limite de 20 fotos.');if(typeof body.content!=='string'||body.content.length>2800000||!/^data:image\/(webp|jpeg|png);base64,[A-Za-z0-9+/]+=*$/.test(body.content))fail(400,'Prepare a imagem antes do envio.');const bytes=Buffer.from(body.content.split(',')[1],'base64');let output;try{const image=sharp(bytes,{limitInputPixels:20000000,failOn:'warning'});const m=await image.metadata();if(m.pages>1||!['webp','jpeg','png'].includes(m.format)||Math.max(m.width,m.height)<600||Math.min(m.width,m.height)<200)fail(400,'Fotografia inválida ou pequena demais.');output=await image.rotate().resize({width:3200,height:3200,fit:'inside',withoutEnlargement:true}).webp({quality:90}).toBuffer({resolveWithObject:true});}catch(e){if(e.status)throw e;fail(400,'Não foi possível preparar a foto.');}if(output.data.length>3000000)fail(400,'Reduza o tamanho da foto.');const id=randomUUID();await request('/storage/v1/object/eme-property-photos/'+id+'.webp',{method:'POST',raw:true,body:output.data,headers:{'Content-Type':'image/webp','x-upsert':'false'}});const prepared={id,width:output.info.width,height:output.info.height,caption,room,...(source?{originalId:source.originalId||source.id,originalWidth:source.originalWidth||source.width,originalHeight:source.originalHeight||source.height,enhancement:body.method==='lanczos3-2x'?'Ampliação convencional 2×':'ESRGAN Slim 2×',enhancedAt:new Date().toISOString(),reviewedBy:user.id}:{})};data=changed({...data,photos:source?data.photos.map(p=>p.id===source.id?prepared:p):[...data.photos,prepared]});try{row=await save(req,row,data,'Fotografia adicionada','Nova fotografia recebida. Curadoria requer nova conferência.');}catch(e){try{await request('/storage/v1/object/eme-property-photos',{method:'DELETE',body:{prefixes:[id+'.webp']}});}catch{console.error('Private orphan photo requires cleanup');}throw e;}return send(201,listing(row));
    }else fail(405,'Operação indisponível.');
   }
