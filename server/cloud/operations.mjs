...
export function attachCloudOperations({client,hashOf}){
 ...
 const metadata=async user=>{const [members,properties]=await Promise.all([rest('eme_profiles?select=id,name,role,active&order=name'+(user.role==='admin'?'':'&id=eq.'+user.id)),rest('eme_cases?select=id,assignee_id,data->>title&order=created_at.desc'+(user.role==='admin'?'':'&assignee_id=eq.'+user.id))]);return {members,properties:properties.map(item=>({id:item.id,title:item.title||'Imóvel sem título',assigneeId:item.assignee_id}))};};
