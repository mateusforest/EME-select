...
export function attachCloudIntelligence({ client, hashOf, caseFor, env = process.env, fetcher }) {
  ...
  const properties = user => rest('eme_cases?id=neq.'+developmentRecordId+'&select=id,version,data->>title&order=updated_at.desc' + (user.role === 'admin' ? '' : '&assignee_id=eq.' + user.id));
  const store = {
