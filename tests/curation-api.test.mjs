...
test('curation: evidence, review gates, decisions, revisions and migration',async t=>{
  ...
  await t.test('existing schema migrates without changing accounts or evaluations',async()=>{
    ...
    const check=new DatabaseSync(dbPath,{readOnly:true});assert.equal(check.prepare('PRAGMA user_version').get().user_version,2);assert.equal(check.prepare('SELECT count(*) n FROM users').get().n,before);check.close();
  ...
  await t.test('high score never bypasses documentary review; broker cannot sign off',async()=>{
    ...
    await update(p,broker);assert.equal(data.curation.score,100);assert.equal(data.curation.blockers.length,4);
  ...
  await t.test('minimum dimension and unresolved issues block the queue',async()=>{
    const p=payload();p.criteria[0].score=2;await update(p);assert.equal(data.curation.score,82);assert.equal((await decide('submit')).status,409);
  ...
  await t.test('approval requires review stage, admin and human acknowledgement',async()=>{
    ...
    assert.equal(data.evaluation.stage,'Entrada aprovada');assert.equal(data.curation.locked,true);assert.equal(data.history[0].author,'Admin Teste');assert.match(data.history[0].detail,/sem publicação automática/);
